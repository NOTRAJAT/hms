...
import { SalonPackageOption, ServiceTransaction } from '../../../core/models/service.model';
...
export class SalonServiceComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    slotDateTime: ['', [Validators.required]],
  ...
  onBookingChange(): void {
    ...
    const currentSlot = String(this.form.value.slotDateTime ?? '');
    if (currentSlot) {
      return;
    }
    const checkIn = new Date(`${booking.checkInDate}T10:00:00`);
    const now = new Date();
    const suggested = checkIn > now ? checkIn : now;
    suggested.setSeconds(0, 0);
    this.form.patchValue({ slotDateTime: this.toDateTimeLocal(suggested) });
  }
    ...
    this.bookingApi.list(userId).subscribe({
      next: (bookings) => {
        ...
        this.serviceApi.catalog().subscribe({
          next: (catalog) => {
            this.packages = catalog.salonPackages ?? [];
            this.loading = false;
          },
          error: () => {
            this.loading = false;
  ...
  private createRequest(): void {
    ...
    this.serviceApi.createSalon({
      bookingId: booking.bookingId,
      packageCode: String(this.form.value.packageCode ?? ''),
      slotDateTime: String(this.form.value.slotDateTime ?? ''),
      ...
      next: (result) => {
        ...
        this.message = `Salon booking confirmed. Request ID: ${result.requestId}`;
        this.stage = 'card';
        this.submitting = false;
        this.isMockLoading = false;
        this.form.patchValue({ otp: '' });
      ...
      error: (error) => {
        ...
        this.isMockLoading = false;
        this.messageType = 'error';
        this.message = error?.error?.error || 'Unable to submit salon request right now.';
      }
    });
  }
  ...
  private isCardStageValid(): boolean {
    return !!this.form.get('bookingId')?.valid
      && !!this.form.get('packageCode')?.valid
      && !!this.form.get('slotDateTime')?.valid
  ...
  private markCardFieldsTouched(): void {
    ['bookingId', 'packageCode', 'slotDateTime', 'paymentMethod', 'cardholderName', 'cardNumber', 'expiryDate', 'cvv', 'billingAddress']
